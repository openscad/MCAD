...
module metric_ruler(millimeters)
{
  difference()
  {
    ...
    for (i = [0:length_cm(1):length_mm(millimeters) + epsilon]) {
      translate([ i, length_cm(2.5), length_mm(0.75) ]) color("Red") cube(
        size =
          [ length_mm(0.5), length_cm(1) + epsilon, length_mm(0.5) + epsilon ],
        center = true);
    }
    // Half centimeter markings
    for (i = [length_cm(0.5):length_cm(1):length_mm(millimeters) + epsilon]) {
      translate([ i, length_cm(2.7), length_mm(0.875) ]) color("Red")
        cube(size =
               [
                 length_mm(0.5),
