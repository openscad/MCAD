 ...
 * cut-off Version 1.2.  2012-09-09   Use discrete polyhedra rather than
 * linear_extrude() Version 1.1.  2012-09-07   Corrected to right-hand threads!
 */
...
module test_threads($fa = 5, $fs = 0.1)
{
  ...
  translate([ 10, 0, 0 ]) square_thread(8, 1.5, 10);

  translate([ 20, 0, 0 ]) acme_thread(8, 1.5, 10);

  translate([ 30, 0, 0 ]) buttress_thread(8, 1.5, 10);

  translate([ 40, 0, 0 ]) english_thread(1 / 4, 20, 1);
...
module metric_thread(diameter = 8,
                     pitch = 1,
                     length = 1,
                     internal = false,
                     n_starts = 1)
{
  trapezoidal_thread(pitch = pitch,
                     length = length,
                     upper_angle = 30,
                     lower_angle = 30,
...
module square_thread(diameter = 8,
                     pitch = 1,
                     length = 1,
                     internal = false,
                     n_starts = 1)
...
module buttress_thread(diameter = 8,
                       pitch = 1,
                       length = 1,
                       internal = false,
                       n_starts = 1,
                       ...
                       pitch_depth_ratio = 3 /
                                           2 // ratio of pitch to thread depth
)
{
  trapezoidal_thread(pitch = pitch,
                     length = length,
 ...
 */
module trapezoidal_thread(pitch,
                          length,
                          upper_angle,
                          lower_angle,
                          outer_flat_length,
                          ...
                          n_starts = 1)
{
  ...
  */
  ...
  path_transforms = [for (i = [0:slices + length2twist(pitch) / fa])
      let(a = i * fa)(rotation(axis = [ 0, 0, a ]) *
  ...
  difference()
  {
    ...
    translate([ 0, 0, length + pitch / 2 ]) cube(
      [ major_radius * 2 + .1, major_radius * 2 + .1, pitch ], center = true);

    translate([ 0, 0, -pitch / 2 ]) cube(
      [ major_radius * 2 + .1, major_radius * 2 + .1, pitch ], center = true);
  }
}
...
module english_thread(diameter = 0.25,
                      threads_per_inch = 20,
                      length = 1,
                      internal = false,
                      n_starts = 1)
