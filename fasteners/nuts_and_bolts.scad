...
module mcad_nut_hole(size, tolerance = +0.0001, proj = -1)
{
  ...
  if (proj == -1) {
    ...
    translate([ -radius / 2, 0 ]) square([ radius * 2, height ]);
  }
}

module mcad_bolt_hole(size,
                      length,
                      cap_extra_length,
                      tolerance = +0.0001,
                      proj = -1)
{
  ...
  if (proj == 2) {
    ...
    translate([ -radius, 0 ]) square([ radius * 2, length ]);
  }
}
 ...
 *                                           below_nut, above_nut)
 */
module mcad_bolt_hole_with_nut(size,
                               length,
                               ...
                               nut_tolerance = 0.001)
{
  ...
  elevation = ((align_with == "above_head")
                 ? 0
                 : (align_with == "below_head")
                     ? cap_head_h
                     : (align_with == "center")
                         ? cap_head_h + length / 2
                         : (align_with == "below_nut")
                             ? cap_head_h + length
                             : (align_with == "above_nut")
                                 ? cap_head_h + length + nut_thickness
                                 : 0);
  ...
  translate([ 0, 0, -elevation ])
  {
    ...
    translate([ 0, 0, cap_head_h ])
      mcad_bolt_hole(size = size,
                     length = length + screw_extra_length,
                     cap_extra_length = head_extra_length,
                     tolerance = bolt_tolerance);

    /* nut */
    translate([ 0, 0, cap_head_h + length - epsilon ]) hull()
    {
      axis = (nut_projection == "axial") ? +Z : +X;

      mcad_linear_multiply(
        no = 2, separation = nut_projection_length, axis = axis)
