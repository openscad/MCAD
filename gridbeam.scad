...
module zBeam(segments)
{
  if (mode == "model") {
    difference()
    {
      ...
      for (i = [0:segments - 1]) {
        translate(
          [ beam_width / 2, beam_width + 1, beam_width * i + beam_width / 2 ])
          rotate([ 90, 0, 0 ])
      ...
      if (beam_is_hollow == 1) {
        translate([ beam_wall_thickness, beam_wall_thickness, -1 ]) cube([
...
{
  if (mode == "model") {
    translate([ 0, 0, -beam_shelf_thickness ]) topShelf(width, depth, corners);
  }
