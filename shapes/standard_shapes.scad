...
module _halfCircelSegment(radius, height, angle)
{
  difference()
  {
    union() { cylinder(h = height, r = radius, center = true); }
    union()
    {
      translate([ 0, -radius / 2 - OS, 0 ]) cube(
        [ (radius + OS) * 2, radius + OS, height + 2 * OS ], center = true);
      rotate(a = angle, v = Z) translate([ 0, -radius / 2, 0 ]) cube(
        [ (radius + OS) * 2, radius + OS, height + 2 * OS ], center = true);
    }
  }
}
