...
function 3dtri_sides2coord(a, b, c) = [
    ...
    [ c, 0, 0 ],
    [
        (pow(c, 2) + pow(a, 2) - pow(b, 2)) / (2 * c),
        sqrt(pow(a, 2) - pow((pow(c, 2) + pow(a, 2) - pow(b, 2)) / (2 * c), 2)),
        0
    ]
];
//
...
function 3dtri_centerOfGravityCoord(Acord, Bcord, Ccord) = [
    (Acord[0] + Bcord[0] + Ccord[0]) / 3,
    (Acord[1] + Bcord[1] + Ccord[1]) / 3,
    0
];
//
//
...
function 3dtri_centerOfcircumcircle(Acord, Bcord, Ccord) = [
    0.5 * Bcord[0],
    0.5 * ((pow(Ccord[1], 2) + pow(Ccord[0], 2) - Bcord[0] * Ccord[0]) /
           Ccord[1]),
    0
];
//
...
//
//	COMMENT:  Calculate circumcircle radius of trinagle with round vertices
//having 			radius R = 2 		vertices = 3dtri_sides2coord (3,4,5); 		cc =
//3dtri_centerOfcircumcircle (vertices[0],vertices[1],vertices[2]); 		cr =
//3dtri_radiusOfcircumcircle (vertices[0],cc,2);
// -------------------------------------------------------------------------------------
//
...
function 3dtri_centerOfIn_circle(Acord, Bcord, Ccord, r) = [
    (Bcord[0] + sqrt(pow(Ccord[0] - Bcord[0], 2) + pow(Ccord[1], 2)) +
     sqrt(pow(Ccord[0], 2) + pow(Ccord[1], 2))) /
            2 -
        sqrt(pow(Ccord[0] - Bcord[0], 2) + pow(Ccord[1], 2)),
    r,
    0
];
//
...
//	COMMENT:
//		You might use the result from function 3dtri_sides2coord
//		to call module 3dtri_rnd_draw ( vertices[0],vertices[1],vertices[2], h,
//r)
// -------------------------------------------------------------------------------------
//
module 3dtri_rnd_draw(Acord, Bcord, Ccord, h, r)
{
    ...
    p1 = Bcord + [ 0, -r, 0 ];
    p2 = Bcord + [
        r * Avect[1] / sqrt(pow(Avect[0], 2) + pow(Avect[1], 2)),
        -r * Avect[0] / sqrt(pow(Avect[0], 2) + pow(Avect[1], 2)),
        0
    ];
    p3 = Ccord + [
        r * Avect[1] / sqrt(pow(Avect[0], 2) + pow(Avect[1], 2)),
    ...
    p4 = Ccord + [
        -r * Ccord[1] / sqrt(pow(Ccord[0], 2) + pow(Ccord[1], 2)),
        r * Ccord[0] / sqrt(pow(Ccord[0], 2) + pow(Ccord[1], 2)),
        0
    ];
    p5 = Acord + [
        -r * Ccord[1] / sqrt(pow(Ccord[0], 2) + pow(Ccord[1], 2)),
    ...
    union()
    {
        polyhedron(
            ...
            triangles = [
                bottom_triangles[0], bottom_triangles[1], bottom_triangles[2],
                bottom_triangles[3], A_edge_triangles[0], A_edge_triangles[1],
                c_side_triangles[0], c_side_triangles[1], B_edge_triangles[0],
                B_edge_triangles[1], a_side_triangles[0], a_side_triangles[1],
                C_edge_triangles[0], C_edge_triangles[1], b_side_triangles[0],
                b_side_triangles[1], top_triangles[0],    top_triangles[1],
...
//  echo ("The calculated co-ordinates and radius are show in this console
//  window"); echo ("Geometry rules for a right angle triangle say, that the
//  circumcircle is the"); echo ("Thales Circle which center must be in the
//  middle of the triangle's c - side"); echo
//  ("==========================================="); vertices  =
//  3dtri_sides2coord (30,40,50); echo("A = ",vertices[0],"  B = ",vertices[1],"
//  C = ",vertices[2]); cg = 3dtri_centerOfGravityCoord
//  (vertices[0],vertices[1],vertices[2]); echo (" Center of gravity = ",cg); cc
//  = 3dtri_centerOfcircumcircle (vertices[0],vertices[1],vertices[2]); echo ("
//  Center of circumcircle = ",cc); cr = 3dtri_radiusOfcircumcircle
//  (vertices[0],cc,r); echo(" Radius of circumcircle ",cr); ir =
//  3dtri_radiusOfIn_circle (vertices[0],vertices[1],vertices[2]); echo ("
//  Radius of in-circle = ",ir); ic = 3dtri_centerOfIn_circle
//  (vertices[0],vertices[1],vertices[2],ir); echo (" Center of in-circle =
//  ",ic);
...
//  		scale([0.8,0.8,1]) translate([6,2,4*h])  3dtri_rnd_draw
//  (vertices[0], vertices[1], vertices[2],5*h,r);
//  	     }
//  	     translate (ic+[0,0,5*h])
//  cylinder(h=10*h,r1=ir+r,r2=ir+r,center=true);
//  	}
//  	translate (ic+[0,0,5*h])
//  cylinder(h=12*h,r1=0.5*ir,r2=0.5*ir,center=true);
//  }
