...
module mcad_rounded_box(size, radius, sidesonly, center = false)
{
    ...
    place_xy() for (x = [
                        ...
                        -size[0] / 2 +
                        radius
                    ]) for (y = [ size[1] / 2 - radius, -size[1] / 2 +
                                  radius ]) translate([ x, y, 0 ]) children();

    translate(center ? [ 0, 0, 0 ] : size / 2) hull() if (sidesonly)
    {
        place_xy() cylinder(r = radius, h = size[2], center = true);
    }
    else
    {
        for (z = [ size[2] / 2 - radius, -size[2] / 2 + radius ])
            translate([ 0, 0, z ]) place_xy() sphere(r = radius);
    }
}
...
module ccube(size, center = false)
{
    ...
    translate([ get_offset(0), get_offset(1), get_offset(2) ]) cube(size);
}
...
module teardrop(radius, length, angle)
{
    ...
    {
        linear_extrude(
            height = length, center = true, convexity = radius, twist = 0)
    ...
    /*
            #polygon(points = [[radius * cos(-angle / 2), radius * sin(-angle /
       2), 0],[radius * cos(-angle / 2), radius * -sin(-angle / 2),
       0],[(sin(-angle - 45) + cos(-angle - 45)) * radius, 0, 0]], paths = [[0,
       1, 2]]); #polygon(points = [[radius * -cos(-angle / 2), radius *
       sin(-angle / 2), 0],[radius * -cos(-angle / 2), radius * -sin(-angle /
       2), 0],[(sin(-angle - 45) + cos(-angle - 45)) * radius, 0, 0]], paths =
       [[0, 1, 2]]); #polygon(points = [[radius * sin(-angle / 2), radius *
       cos(-angle / 2), 0],[radius * sin(-angle / 2), radius * -cos(-angle / 2),
       0],[(sin(-angle - 45) + cos(-angle - 45)) * radius, 0, 0]], paths = [[0,
       1, 2]]);
        */
}
...
{
    rotate_extrude() translate([ inner_radius + thickness[0] / 2, 0, 0 ])
        ellipse(width = thickness[0], height = thickness[1]);
}

// wall is wall thickness
module tube(h, r, wall, center = false)
{
    linear_extrude(height = h, center = center) difference()
    {
...
module oval_tube(height, rx, ry, wall, center = false)
{
    difference()
    {
        ...
        translate([ 0, 0, -0.1 ])
#scale([ (rx - wall) / rx, (ry - wall) / rx, 1 ])                              \
    cylinder(h = height + 0.2, r = rx, center = center);
    }
}
...
{
    polyhedron(points = points, triangles = [
        [ 0, 1, 2 ],
        [ 3, 0, 2 ],
        [ 7, 6, 5 ],
        [ 7, 5, 4 ], // Given polygons
        [ 0, 4, 1 ],
        [ 4, 5, 1 ],
        [ 1, 5, 2 ],
        [ 2, 5, 6 ], // Connecting
