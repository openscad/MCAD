...
// TODO: The small motors seem to be a bit too long, I picked the size specs
// from all over the place, is there some canonical reference?
Nema08 = [[NemaModel, 8],
...
function motorScrewSpacing(model = Nema23) =
  lookup(NemaDistanceBetweenMountingHoles, model);

function Nema(number) = NemaDefinitions[number];

module motor(model = Nema23,
             size = NemaMedium,
             dualAxis = false,
             pos = [ 0, 0, 0 ],
             orientation = [ 0, 0, 0 ])
{
  ...
  color(stepperBlack)
  {
    ...
    {
      translate([ -mid, -mid, 0 ]) difference()
      {
        ...
        // Rounded edges
        if (roundR > 0) {
          translate([ mid + mid, mid + mid, length / 2 ]) rotate([ 0, 0, 45 ])
            cube(size = [ roundR, roundR * 2, 4 + length + extrSize + 2 ],
                 center = true);
          translate([ mid - (mid), mid + (mid), length / 2 ])
            rotate([ 0, 0, 45 ])
              cube(size = [ roundR * 2, roundR, 4 + length + extrSize + 2 ],
                   center = true);
          translate([ mid + mid, mid - mid, length / 2 ]) rotate([ 0, 0, 45 ])
            cube(size = [ roundR * 2, roundR, 4 + length + extrSize + 2 ],
                 center = true);
        ...
        {
          translate([ mid + holeDist, mid + holeDist ])
            cylinder(h = holeDepth + 1 * length_mm, r = holeRadius);
          translate([ mid - holeDist, mid + holeDist ])
            cylinder(h = holeDepth + 1 * length_mm, r = holeRadius);
          translate([ mid + holeDist, mid - holeDist ])
            cylinder(h = holeDepth + 1 * length_mm, r = holeRadius);
        ...
        difference()
      {
        ...
        cylinder(h = axleLengthFront + 1 * length_mm,
                 r = axleRadius,
                 $fs = axleRadius / 10);
        ...
        if (axleFlatDepth > 0)
          translate([
            axleRadius - axleFlatDepth,
            -5 * length_mm,
            -extrSize * length_mm - (axleLengthFront - axleFlatLengthFront)
          ]) cube(size = [ 5 * length_mm, 10 * length_mm, axleLengthFront ]);
      }
      ...
      if (dualAxis) {
          ...
          difference()
        {
          ...
          cylinder(h = axleLengthBack + 0 * length_mm,
                   r = axleRadius,
                   $fs = axleRadius / 10);
          ...
          if (axleFlatDepth > 0)
            translate([
              axleRadius - axleFlatDepth,
              -5 * length_mm,
              (axleLengthBack - axleFlatLengthBack)
            ]) cube(size = [ 5 * length_mm, 10 * length_mm, axleLengthBack ]);
        }
      }
    }
  }
