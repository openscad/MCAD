  ...
  translate([ botL / 2, 0, 0 ]) rotate([ 90, 0, 0 ])
  {
    translate([ 0, supportSize / 2, 0 ])
    {
      union()
      {
        ...
        for (i = [0:N - 1]) {
          translate(
            [ i * offset - length / 2 + realBase / 2, 0, thickness / 2 ])
          {
...
{
  // cp= circular pitch - for pinion pitch in mm/per as measured along the ptich
  // radius (~1/2 way up tooth) N= number of teeth width= width of the gear
  // shaft_diameter= diameter of hole for shaft
  // backlash - I think this is just a bodge for making things fit when printed
  ...
  union()
  {
    for (i = [1:res])
      assign(point1 = involute(base_radius,
                               start_angle +
                                 (stop_angle - start_angle) * (i - 1) / res),
             point2 = involute(
               base_radius, start_angle + (stop_angle - start_angle) * i / res))
      {
...
module trapezoid(top, base, height)
{
  ...
  polyhedron(
    points =
      [ basePT1, basePT2, basePT3, basePT4, topPT1, topPT2, topPT3, topPT4 ],
    faces = [
      [ 0, 1, 2 ],
      [ 0, 2, 3 ],
      [ 3, 7, 0 ],
