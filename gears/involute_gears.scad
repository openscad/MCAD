...
// http://www.thingiverse.com/thing:3575 and
// http://www.thingiverse.com/thing:3752
...
translate([ 700 * PI / 180 * 30 / 2 / PI * 2, 0, 0 ]) rotate(180 + 360 / 30 / 2)
  gear(number_of_teeth = 30,
       circular_pitch = 700,
       gear_thickness = 12,
       rim_thickness = 15,
...
//==================================================
// Bevel Gears:
// Two gears with the same cone distance, circular pitch (measured at the cone
// distance) and pressure angle will mesh.
                  ...
                  finish = -1)
{
  ...
  finish = (finish != -1)
             ? finish
             : (pitch_angle < 45) ? bevel_gear_flat : bevel_gear_back_cone;

  apex_to_apex = cone_distance / cos(pitch_angle);
  back_cone_radius = apex_to_apex * sin(pitch_angle);

  // Calculate and display the pitch angle. This is needed to determine the
  // angle to mount two meshing cone gears.
  ...
  back_cone_end_radius = outside_pitch_radius - dedendum * cos(pitch_angle) -
  ...
  half_tooth_thickness =
    outside_pitch_radius * sin(360 / (4 * number_of_teeth)) - backlash / 4;
  ...
  face_cone_end_radius = outside_pitch_radius - face_width / sin(pitch_angle) -
                         face_cone_descent / tan(pitch_angle);

  // For the bevel_gear_flat finish option, calculate the height of a cube to
  // select the portion of the gear that includes the full pitch face.
  ...
  difference()
  {
    intersection()
    {
      union()
      {
        rotate(half_thick_angle) translate([ 0, 0, pitch_apex - apex_to_apex ])
          cylinder($fn = number_of_teeth * 2,
                   r1 = root_cone_full_radius,
                   r2 = 0,
                   h = apex_to_apex);
        ...
        //				for (i = [1:1])
        {
          ...
          {
            involute_bevel_gear_tooth(back_cone_radius = back_cone_radius,
      ...
      if (finish == bevel_gear_back_cone) {
        ...
        translate(
          [ -1.5 * outside_pitch_radius, -1.5 * outside_pitch_radius, 0 ])
          cube([
...
module involute_bevel_gear_tooth(back_cone_radius,
                                 root_radius,
                                 base_radius,
                                 outer_radius,
                                 pitch_apex,
                                 ...
                                 involute_facets)
{
  ...
  translate([ 0, 0, pitch_apex ])
    rotate([ 0, -atan(back_cone_radius / cone_distance), 0 ])
      translate([ -back_cone_radius * 2, 0, -cone_distance * 2 ]) union()
  {
    for (i = [1:res]) {
      assign(
        point1 =
          involute(base_radius * 2,
                   start_angle + (stop_angle - start_angle) * (i - 1) / res),
        point2 = involute(base_radius * 2,
                          start_angle + (stop_angle - start_angle) * (i) / res))
      {
        assign(
            ...
            internal = false)
{
  ...
  twist = ((twist != 0)
             ? twist
             : (tan(helix_angle) * rim_thickness / pitch_circumference * 360));
  echo("Twist: ", twist);
  ...
  module flat_gear()
  {
    ...
    module rounding_circles()
    {
      ...
      circle_positions =
        (internal ? [0:1:number_of_teeth] : [0.5:1:number_of_teeth - 0.5]);

      if (roundsize > 0)
        for (i = circle_positions) {
          rotate([ 0, 0, (i * 360 / number_of_teeth) ])
            translate([ path_radius, 0 ])
              circle(r = ((360 / number_of_teeth - half_thick_angle) / 360) *
                         pi * root_radius / 2 * roundsize,
                     $fa = 18,
                     $fs = 0.5);
        }
    }
  ...
  {
    lower_rim_thickness =
      (rim_thickness / 2 + ((internal && gear_thickness == 0) ? 0.1 : 0));
    lower_twist = twist / rim_thickness * lower_rim_thickness;

    upper_rim_thickness = (rim_thickness / 2 + ((internal) ? 0.1 : 0));
    ...
    if (flat) {
      ...
      {
        linear_extrude(height = upper_rim_thickness,
                       convexity = 10,
                       twist = upper_twist) flat_gear();

        mirror([ 0, 0, 1 ]) linear_extrude(height = lower_rim_thickness,
                                           convexity = 10,
                                           twist = lower_twist) flat_gear();
      }

    } else {
      linear_extrude(height = rim_thickness + (internal ? 0.2 : 0),
                     convexity = 10,
                     twist = twist) flat_gear();
    }
  }
  ...
  module ensure_rim()
  {
    if (flat) {
      ...
      difference()
      {
        linear_extrude(height = rim_thickness) circle(r = rim_radius);

        translate([ 0, 0, gear_thickness ]) children();
      }
      ...
      difference()
      {
        ...
        translate([ 0, 0, gear_thickness ]) linear_extrude_flat_option(
          flat = flat, height = (rim_thickness - gear_thickness + 0.1))
          circle(r = rim_radius);
      }
    }
  }
  ...
  module bore() { circle(d = bore_diameter); }

  difference()
  {
    union()
    {
      ensure_rim() extruded_gear();

      hub();
    }

    linear_extrude_flat_option(
      flat = flat, center = true, height = (max_thickness + 0.1) * 2) union()
    {
      _circles();
      bore();
    }
...
module linear_extrude_flat_option(flat = false,
                                  height = 10,
                                  center = false,
                                  convexity = 2,
                                  twist = 0)
{
  if (flat == false) {
    linear_extrude(
      height = height, center = center, convexity = convexity, twist = twist)
      children();
  } else {
    children();
  }
                           ...
                           involute_facets)
{
  ...
  pitch_point =
    involute(base_radius, involute_intersect_angle(base_radius, pitch_radius));
  pitch_angle = atan2(pitch_point[1], pitch_point[0]);
  centre_angle = pitch_angle + half_thick_angle;
  ...
  function reverse(v) = [for (i = [1:len(v)]) v[len(v) - i]];

  side1_points = [for (i = [0:res]) rotate_2dvector(
    centre_angle,
    involute(base_radius, start_angle + (stop_angle - start_angle) * i / res))];

  side2_points = [for (i = reverse(side1_points)) mirror_2dvector(i)];
...
function involute(base_radius, involute_angle) = [
  base_radius *
    (cos(involute_angle) + involute_angle * pi / 180 * sin(involute_angle)),
  base_radius*(sin(involute_angle) -
               involute_angle * pi / 180 * cos(involute_angle))
];
...
#gear(number_of_teeth = 11,
        ...
        translate([ 0, 0, 8 ])
        {
          gear(number_of_teeth = 6,
               ...
               pressure_angle = 31);
#rotate([ 0, 0, 360 * 5 / 6 ])
          translate([ 22.5, 0, 1 ]) gear(number_of_teeth = 21,
                                         circular_pitch = 300,
                                         bore_diameter = 2,
                                         hub_diameter = 4,
                                         rim_width = 1,
...
test_backlash()
{
  ...
  translate([ -29.166666, 0, 0 ])
  {
    ...
    rotate([ 0, 0, 360 / teeth / 4 ]) gear(number_of_teeth = teeth,
                                           circular_pitch = 700,
                                           gear_thickness = 12,
                                           rim_thickness = 15,
                                           rim_width = 5,
  ...
  color([ 0, 0, 128, 0.5 ]) translate([ 0, 0, -5 ])
