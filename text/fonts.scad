...
module outline_2d(outline, points, paths, width = 0.1, resolution = 8)
{
  if (outline && resolution > 4) {
    for (j = [0:len(paths) - 1])
      union()
      {
        for (i = [1:len(paths[j]) - 1])
          hull()
          {
            translate(points[paths[j][i - 1]])
              circle($fn = resolution, r = width / 2);
            translate(points[paths[j][i]])
              circle($fn = resolution, r = width / 2);
          }
        hull()
        {
          translate(points[paths[j][len(paths[j]) - 1]])
            circle($fn = resolution, r = width / 2);
          translate(points[paths[j][0]])
            circle($fn = resolution, r = width / 2);
        }
      }
  } else {
    ...
    if (bold) {
      union()
      {
        ...
        for (i = [0:resolution - 1])
          assign(dx = width * cos(360 * i / resolution),
                 dy = width * sin(360 * i / resolution)) translate([ dx, dy ])
            child(j);
      }
    } else {
      child(j);
                ...
                strike_width = 1.0)
{
  ...
  for (i = [0:len(char_index_map) - 1])
    assign(thisCharIndex = char_index_map[i],
           x_pos = i * size + line_shift_x * size / char_width)
    {
      translate([ x_pos, line * size + char_shift_height * size / char_height ])
        scale([ size / char_width, size / char_height ])
      {
        if (char_thickness == 0)
          bold_2d(bold, width = bold_width, resolution = bold_resolution)
            outline_2d(outline,
                       points = font[2][thisCharIndex][6][0],
                       paths = font[2][thisCharIndex][6][1],
                       width = outline_width,
                       resolution = outline_resolution);
        if (charstring[i] != " ") {
          if (underline)
            translate(underline_start) square(
              size = [ char_width - 2 * underline_start[0], underline_width ],
              center = false);
          if (strike)
            translate([ strike_start[0], char_height / 2 + strike_start[1] ])
              square(size = [ char_width - 2 * strike_start[0], strike_width ],
                     center = false);
        }
        if (char_thickness > 0)
          polyhedron(points = font[2][thisCharIndex][6][0],
                     triangles = font[2][thisCharIndex][6][1]);
      }
    }
}
...
//
//   Section 3.2 of Specification 800 (Braille Books and Pamphlets) February
//   2008 reads as follows:
//     Size and Spacing
//     3.2.1	The nominal height of braille dots shall be 0.019 inches [0.48 mm]
//     and shall be uniform within any given transcription. 3.2.2	The nominal
//     base diameter of braille dots shall be 0.057 inches [1.44 mm]. 3.2.3 Cell
//     spacing of dots shall conform to the following: 3.2.3.1	The nominal
//     distance from center to center of adjacent dots (horizontally or
//     vertically, but not diagonally)
//              in the same cell shall be 0.092 inches [2.340 mm].
//     3.2.3.2	The nominal distance from center to center of corresponding dots
//     in adjacent cells shall be 0.245 inches [6.2 mm]. 3.2.4    The nominal
//     line spacing of braille cells from center to center of nearest
//     corresponding dots in adjacent lines shall
//              be 0.400 inches [1.000 cm].
//
// Additional References:
//    http://www.loc.gov/nls/specs/800.pdf
//    http://www.tiresias.org/research/reports/braille_cell.htm
module braille_ascii_spec800(inString,
                             dot_backing = true,
                             cell_backing = false,
                             justify = 1,
                             align = -1,
                             ...
                             dot_d = 1.44,
                             dot_spacing = 2.340,
                             cell_d2d_spacing = 6.2,
                             line_d2d_spacing = 10.0,
                             echo_translate = true)
{
  // justify:
  //  -1 : left side
  ...
  thisFont = braille_ascii_font(dot_h = dot_h,
                                dot_d = dot_d,
                                dot_spacing = dot_spacing,
                                cell_d2d_spacing = cell_d2d_spacing,
                                line_d2d_spacing = line_d2d_spacing);
  x_shift = thisFont[0][0];
  y_shift = thisFont[0][1];
  theseIndicies = search(inString, thisFont[2], 1, 1);
  for (i = [0:len(theseIndicies) - 1])
    translate([
      i * x_shift - (1 - justify) * x_shift * len(theseIndicies) / 2,
      -y_shift * (align + 1) / 2
    ]) assign(dotPattern = thisFont[2][theseIndicies[i]][6])
    {
      if (dot_backing)
        translate([
          cell_d2d_spacing / 2 - dot_spacing / 2 - dot_d / 2,
          line_d2d_spacing / 2 - dot_spacing - dot_d / 2,
          -dot_h
        ]) cube(size = [ dot_spacing + dot_d, 2 * dot_spacing + dot_d, dot_h ],
                center = false);
      if (cell_backing)
        translate([ 0, 0, -dot_h ])
      ...
      if (echo_translate)
        echo(str(
          inString[i], " maps to '", thisFont[2][theseIndicies[i]][4], "'"));
      for (dotIndex = dotPattern) {
        translate([
          cell_d2d_spacing / 2 - dot_spacing / 2 +
            floor((dotIndex - 1) / 3) * dot_spacing,
          line_d2d_spacing / 2 - dot_spacing + (2 - (dotIndex - 1) % 3) *
          dot_spacing
...
//  2 5
//  3 6
function braille_ascii_font(dot_h = 0.48,
                            dot_d = 1.44,
                            dot_spacing = 2.340,
                            cell_d2d_spacing = 6.2,
                            line_d2d_spacing = 10.0) =
  [[cell_d2d_spacing, line_d2d_spacing, 0, "bump"],
   ["Decimal Byte",
    "Caret Notation",
