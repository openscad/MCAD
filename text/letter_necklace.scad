...
module
letter(char, block_size, height, hole_diameter)
{
  union()
  {
    ...
    translate(v = [ 0, 0, (hole_diameter * 1.3) / 2 ])
    {
      color([ 0, 0, 1, 1 ])
      {
        difference()
        {
          cube(size = [ block_size * 8, block_size * 8, hole_diameter + 2 ],
               center = true);
          rotate([ 90, 0, 0 ]) cylinder(
            h = block_size * 8 + 1, r = hole_diameter / 2, center = true);
        }
      }
    }
  }
...
union()
{
  for (column = [0:1]) {
    for (row = [0:4]) {
      translate(
        v =
          [ column * (block_size * 1.1) * 8, row * (block_size * 1.1) * 8, 0 ])
