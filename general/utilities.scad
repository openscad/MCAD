...
function angleOfNormalizedVector(n) =
  [ 0, -atan2(n[2], distance1D([ n[0], n[1] ])), atan2(n[1], n[0]) ];
...
function mirror_2dvector(coord) = [ coord[0], -coord[1] ];
...
function circel_radius3Points(a, b, c) = (distance2D(a, b) * distance2D(b, c) *
                                          distance2D(c, a)) /
                                         (4 * triangle_area3Points(a, b, c));

function circel_radius3lengths(a, b, c) = (a * b * c) /
                                          (4 * triangle_area3lengths(a, b, c));

/********************** triangle **************************/
// Satz des Heron
function triangle_area3lengths(a, b, c) = sqrt((a + b + c) * (a + b - c) *
                                               (b + c - a) * (c + a - b)) /
                                          4;

function triangle_area3Points(a, b, c) =
  triangle_area3lengths(distance2D(a, b), distance2D(b, c), distance2D(c, a));

/************************** coordinate systems translation ****/
//[radius,phi] <=> [x,y]
function conv2D_cartesian2polar(x) =
  [ distance1D(x[0], x[1]), atan2(x[1], x[0]) ];
...
module fromTo(from = [ 0, 0, 0 ],
              ...
              name = "",
              endExtras = [ 0, 0 ],
              endCaps = [ FlatCap, FlatCap ],
              rotation = [ 0, 0, 0 ],
              printString = true)
{

  angle = angleBetweenTwoPoints(from, to);
  length = distance(from, to) + endCaps[0] * size[0] + endCaps[1] * size[0] +
           endExtras[0] + endExtras[1];
  ...
  if (length > 0) {
    ...
    color(material) translate(from) rotate(angle) translate([
      -endCaps[0] * size[0] - endExtras[0],
      size[0] * (-0.5 - align[0]),
      size[1] * (-0.5 + align[1])
...
module mirror_duplicate(plane = [ 0, 0, 1 ]) union()
{
  child();
...
module linear_extrude_if(condition,
                         height,
                         center = undef,
                         convexity = undef,
                         twist = undef,
